...
import com.indiegen.game.actors.GameCharacter;
...
public class CustomScreen implements Screen, GestureDetector.GestureListener, CustomCallBack, Levels, InputProcessor {
    ...
    private GameCharacter floor;
    private Player player;
    private CustomActor actingActor;
    private CustomActor dummy;
    private ArrayList<CustomActor> actors;
    private ArrayList<CustomActor> ready;
    ...
    public CustomScreen(Game game) {
        ...
        floor = new GameCharacter(texture);
        dummy = new GameEnemy(enemyTexture, 9999, 9999, "dummy");
        ...
        mydialog = new CustomDialog(this, uiStage);
    ...
    public void initScreen() {
        ...
        actors.add(new GameEnemy(enemyTexture, margin * 4, margin * 5, "1"));
        actors.add(new GameEnemy(enemyTexture, margin * 10, margin * 6, "2"));
        actors.add(new GameEnemy(enemyTexture, margin * 5, margin * 5, "3"));
        actors.add(new GameEnemy(enemyTexture, margin * 3, margin, "4"));
    ...
    private void removeDead() {
        Iterator<CustomActor> itr = ready.listIterator();
        while (itr.hasNext()) {
            ...
            if (player.getPlayerState() == GamePlayerState.WAITING_TO_MOVE) {
                for (RectangleUtils rect : player.getRects()) {
                    if (rect.contains(touchVec.x, touchVec.y)
                        ...
                        AssetsManager.getWalkSound().play();
                        player.setFatigue(player.getWALK());
                    ...
                    if (rect.contains(touchVec.x, touchVec.y)) {
                        for (CustomActor actor : actors) {
                            if (actor.getRectangle().contains(touchVec.x, touchVec.y) && !Objects.equals(actor.getName(), player.getName())) {
                                ...
                                actingActor = actor;
                                player.setFatigue(player.getATTACK());
